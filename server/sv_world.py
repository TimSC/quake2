...
"""
class areanode_t(object):

	def __init__(self):
		self.clear()

	def clear(self):
		self.axis = None # int, -1 = leaf node
		self.dist = None #float
		self.children = [None, None] #struct areanode_s	*[2]
		self.trigger_edicts = None # link_t
		self.solid_edicts = None #link_t

AREA_DEPTH = 4
AREA_NODES = 32

sv_areanodes = []
for i in range(AREA_NODES):
	sv_areanodes.append(areanode_t())
sv_numareanodes:int = None
"""
...
===============
"""
def SV_CreateAreaNode (depth: int, mins: q_shared.vec3_t, maxs: q_shared.vec3_t)->areanode_t:
...
// link to PVS leafs
	...
	for (i=0 ; i<num_leafs ; i++)
	{
		...
		{	// doors may legally straggle two areas,
			...
			{
				if (ent.areanum2 && ent.areanum2 != area && sv_init.sv.state == ss_loading)
...
int SV_PointContents (vec3_t p)
{
	...
	contents = CM_PointContents (p, sv_init.sv.models[1].headnode);
